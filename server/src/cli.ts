...
async function withServices<T>(fn: (svc: Services) => Promise<T>): Promise<T> {
  ...
  try {
    return await fn(createServices(handle.db, config));
  } finally {
    await handle.close();
  }
}
...
bundle
  ...
  .requiredOption("--description <description>", "One-line summary.")
  .action(async (slug: string, opts: { title: string; description: string }) => {
    const created = await withServices((svc) =>
      svc.bundleService.create({ slug, title: opts.title, description: opts.description }),
    );
...
program
  ...
  .option(
    "--enable-bundle-storage",
    "Mirror DB contents to filesystem (env BUNDLE_STORAGE_ENABLED)",
  )
  .option(
    "--bundle-storage-path <path>",
    "Directory for filesystem bundle mirror (env BUNDLE_STORAGE_PATH)",
  )
  .action(
    async (opts: {
      host: string;
      ...
      if (opts.enableBundleStorage === true) set("BUNDLE_STORAGE_ENABLED", "true");
      set("BUNDLE_STORAGE_PATH", opts.bundleStoragePath);
