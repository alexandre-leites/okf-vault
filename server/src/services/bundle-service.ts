...
export class BundleService {
  ...
  async create(input: CreateBundleInput): Promise<BundleRow> {
    ...
    const row = await this.bundles.insert({
      parentId,
      slug,
      title: input.title,
      description: input.description,
    });
    const rootSlug = input.parentPath ? input.parentPath.split("/")[0]! : slug;
    await this.fsSync?.syncBundle(rootSlug);
    return row;
  }

  /** Updates the title and description of an existing bundle (by path). */
  async update(path: string, input: UpdateBundleInput): Promise<BundleRow> {
    const segments = path.split("/").filter(Boolean);
    const { bundle } = await this.resolve(segments);
    const updated = await this.bundles.updateMetadata(bundle.id, input.title, input.description);
    if (!updated) throw new BundleNotFoundError(path);
