 ...
 */
export function createMcpServer(db: Database, log: Logger, config?: Config): McpServer {
  ...
  server.registerTool(
    "okf_bundle_create",
    {
      ...
      inputSchema: {
        ...
        title: z.string().min(1).max(500).describe("Human-readable display title."),
        description: z.string().min(1).max(2000).describe("One-line summary of the bundle."),
      },
    },
    async ({ slug, title, description }) =>
      runTool(async () => {
        log.debug({ slug }, "MCP okf_bundle_create");
        const b = await bundles.create({ slug, title, description });
  ...
  server.registerTool(
    ...
    {
      title: "Update bundle",
      description: "Update the title and description of an existing bundle.",
      inputSchema: {
        slug: z.string().describe("Slug of the bundle to update (e.g. 'global' or 'okf-vault')."),
        title: z.string().min(1).max(500).describe("New human-readable display title."),
        description: z.string().min(1).max(2000).describe("New one-line summary of the bundle."),
      },
    },
    async ({ slug, title, description }) =>
      runTool(async () => {
        log.debug({ slug }, "MCP okf_bundle_update");
        const b = await bundles.update(slug, { title, description });
        return json({ slug: b.slug, title: b.title, description: b.description });
      }),
  );
  ...
  server.registerTool(
    "okf_concept_upsert",
    {
      ...
      inputSchema: {
        ...
        title: z.string().min(1).max(500).describe("Display title."),
        description: z.string().min(1).max(2000).describe("One-line summary."),
        resource: z
          .string()
          .max(2000)
          .optional()
...
export async function serve({ config, log, db, onShutdown }: ServeOptions): Promise<void> {
  ...
  // Initial filesystem sync (non-blocking)
  const svc = createServices(db, config);
  if (svc.fsSyncService) {
    svc.fsSyncService
      .syncAll()
      .catch((err: unknown) => log.error({ err }, "Initial FS sync failed"));
  }
  ...
  const handleMcp = async (c: { req: { raw: Request }; env: unknown }) => {
    ...
    if (!transport) {
      ...
      const server = createMcpServer(db, log, config);
  ...
  if (enableStdio) {
    const stdioServer = createMcpServer(db, log, config);
