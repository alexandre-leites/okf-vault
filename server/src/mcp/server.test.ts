...
suite("MCP server", () => {
  ...
  it("okf_bundle_create rejects missing required fields", async () => {
    const bareSlug = `${slug}-bare`;
    const result = await client.callTool({
      name: "okf_bundle_create",
      arguments: { slug: bareSlug },
    });
    const txt = result.content[0]!.text;
    expect(txt).toContain("MCP error");
  });
  ...
  it("okf_bundle_create with the reserved 'global' slug returns the canonical global bundle", async () => {
    const data = await callTool(client, "okf_bundle_create", {
      slug: "global",
      title: "Global",
      description: "Global knowledge bundle",
    });
  ...
  it("okf_concept_upsert rejects missing required fields", async () => {
    const result = await client.callTool({
      name: "okf_concept_upsert",
      arguments: { bundle: slug, path: "mcp/minimal-concept", type: "reference" },
    });
    const txt = result.content[0]!.text;
    expect(txt).toContain("MCP error");
  });
  ...
  it("okf_concept_search clamps a long snippet that has no word boundary", async () => {
    await callTool(client, "okf_concept_upsert", {
      ...
      type: "reference",
      title: "Nospace",
      description: "Nospace snippet test",
  ...
  it("okf_concept_backlinks finds inbound references", async () => {
    await callTool(client, "okf_concept_upsert", {
      ...
      type: "reference",
      title: "Linked Target",
      description: "Linked target test",
    });
  ...
  it("okf_concept_history lists versions after update", async () => {
    await callTool(client, "okf_concept_upsert", {
      ...
      description: "Updated links test",
