...
suite("REST API", () => {
  ...
  it("POST /okf/bundles creates a bundle and returns 201", async () => {
    const res = await app().request("/okf/bundles", {
      ...
      body: JSON.stringify({ slug, title: "API Test", description: "API test bundle" }),
    });
  ...
  it("DELETE /okf/bundles/{slug} soft-deletes and returns 204", async () => {
    ...
    await bundles.create({ slug: delSlug, title: "To Delete", description: "Bundle to delete" });
  ...
  it("GET /okf/bundles/{slug}/{concept}.md reads a concept", async () => {
    await concepts.create(slug, "read-test", {
      type: "reference",
      title: "Read Me",
      description: "Read test concept",
    });
  ...
  it("PUT /okf/bundles/{slug}/{concept}.md updates a concept", async () => {
    await concepts.create(slug, "update-test", {
      type: "reference",
      title: "Before",
    ...
    const res = await app().request(`/okf/bundles/${slug}/update-test.md`, {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ type: "Playbook", title: "After", description: "After update" }),
    });
    expect(res.status).toBe(200);
    const text = await res.text();
  ...
  it("PUT from markdown when content-type is text/markdown", async () => {
    await concepts.create(slug, "put-md", {
      type: "reference",
      title: "PutBefore",
      description: "Put markdown before",
    });
  ...
  it("DELETE /okf/bundles/{slug}/{concept}.md soft-deletes a concept", async () => {
    await concepts.create(slug, "delete-me", {
      type: "reference",
      title: "Delete Me",
      description: "Concept to delete",
    });
  ...
  it("POST duplicate bundle slug returns 409", async () => {
    const res = await app().request("/okf/bundles", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ slug, title: "Duplicate", description: "Duplicate bundle" }),
    });
  ...
  it("allows body within size limit", async () => {
    ...
    const res = await strict.request(`/okf/bundles/${slug}/small-concept.md`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ type: "reference", title: "Small", description: "Small concept" }),
    });
    expect(res.status).toBe(201);
  });
        ...
        operations: [
          {
            ...
            type: "Note",
            title: "Batch Created",
            description: "Batch created concept",
          },
          {
            method: "POST",
            bundle: slug,
            ...
            type: "Note",
            title: "To Update",
            description: "Batch concept to update",
          },
          {
            method: "PUT",
            bundle: slug,
            ...
            type: "Playbook",
            title: "Batch Updated",
            description: "Batch updated concept",
          },
        ],
      }),
    });
  ...
  it("POST /okf/batch PUT with all optional fields and minimal fields", async () => {
    await concepts.create(slug, "batch-full", {
      type: "Note",
      title: "Full",
      description: "Full test",
    });
    await concepts.create(slug, "batch-min", {
      type: "Note",
      title: "Min",
