...
OKF Vault is a monorepo with the server under [`server/`](../server/). All source
paths in this document are relative to `server/`.

The server stores, serves, and searches Open Knowledge Format (OKF) markdown
documents in PostgreSQL. Three controllers share one service layer:
...
### Controller layer (source in `server/src/`)
...
- **MCP server** (`src/mcp/server.ts`) — MCP SDK with 13 tools, 1 resource template (`okf://{bundle}/{path+}`), 1 prompt (`create_concept`)
...
Implemented as inline routes in `server/src/api/server.ts:extractLinks()` and `findBacklinks()`.

## MCP integration

### Transport

Configured via `MCP_TRANSPORT` env:

- **`both`** (default) — Stdio + Streamable HTTP both active
- **`stdio`** — Standard MCP stdio transport only
- **`http`** / **`sse`** (legacy alias) — Streamable HTTP transport only, reuses the REST API server on `/mcp`
...
Config file locations (in order): `./okfvault.yaml`, `~/.config/okfvault/config.yaml`, `$OKFVAULT_CONFIG`.
...
2. **Production** — `node:22-alpine`, installs `tini`, copies `dist/` and `drizzle/`, runs `node dist/cli.js serve` (all paths relative to `server/`)
